...
class ModelExtensionModuleBrainyFilter extends Model 
{
    ...
    public function enableMod($enable = true)
    {
        ...
        $mod = $this->$modification_path->getModificationByCode('brainyfilter');
        if ($mod) {
            if ((bool)$mod['status'] !== $enable) {
                if ($enable) {
                    $this->$modification_path->enableModification($mod['modification_id']);
                } else {
                    $this->$modification_path->disableModification($mod['modification_id']);
                }
            }
            return true;
        }
