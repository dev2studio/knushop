...
class ModelExtensionModuleSeoUrlGenerator extends Model {
	...
	*/
	...
	public function getURL($query_key, $essence_id) {
		$this->stdelog->write(3, 'getURL() is called');

		$sql = "SELECT `keyword` FROM `" . DB_PREFIX . "url_alias` WHERE `query` = '" . $this->db->escape($query_key) . "=" . (int)$essence_id . "'";
		...
		if ($res->num_rows > 1) {
		  $this->stdelog->write(1, 'getURL() : essence already has more than 1 SEO URL! Delete it!!!');
		
			$sql = "DELETE FROM `" . DB_PREFIX . "url_alias` WHERE `query` = '" . $this->db->escape($query_key) . "=" . (int)$essence_id . "'";
			
			$this->stdelog->write(4, $sql, 'getURL() : $sql DELETE Doubles');
			
			$query = $this->db->query($sql);
			
			$this->stdelog->write(4, $query, 'getURL() : $query DELETE Doubles');
			
			return false;
			
		} elseif ($res->num_rows == 1) {
			$keyword = trim($res->row['keyword']);
			
			if ($keyword) {
				$this->stdelog->write(3, $keyword, 'getURL() : $keyword');
				return $keyword;
			} else {
				$this->stdelog->write(1, $keyword, 'getURL() : $keyword was empty');
				
				$sql = "DELETE FROM `" . DB_PREFIX . "url_alias` WHERE `query` = '" . $this->db->escape($query_key) . "=" . (int)$essence_id . "'";
			
				$this->stdelog->write(4, $sql, 'getURL() : $sql DELETE EMPTY Keyword');

				$query = $this->db->query($sql);

				$this->stdelog->write(4, $query, 'getURL() : $query DELETE EMPTY Keyword');
			} 
		} else {
			$this->stdelog->write(3, 'getURL() : $res->num_rows 0');
			return false;
		}

		$this->stdelog->write(1, 'getURL() : unexpected error');
		return false;
	}
	...
	public function getEssenceName($essence, $primary_key, $essence_id, $language_id) {
		...
		if ($query) {
			$this->stdelog->write(3, $query->row[$column_name], 'getEssenceName() : return $query->row[$column_name]');
	...
	public function makeUniqueUrl($keyword) {
		$this->stdelog->write(3, 'getUniqueUrl() is called');
		...
		$this->stdelog->write(3, $unique_keyword, 'getUniqueUrl() : return $unique_keyword');
	...
	public function getManufacturerNameById($manufacturer_id, $language_id) {
		...
		if ('OpenCart' == $this->config->get('seo_url_generator_system') || 'OpenCart.PRO' == $this->config->get('seo_url_generator_system')) {
			$sql = "SELECT `name` FROM `" . DB_PREFIX . "manufacturer` WHERE `manufacturer_id` = '" . (int)$manufacturer_id . "'";
			
			$query = $this->db->query($sql);
		} else {
			$sql = "SELECT `name` FROM `" . DB_PREFIX . "manufacturer_description` WHERE `manufacturer_id` = '" . (int)$manufacturer_id . "' AND `language_id` = '" . (int)$language_id . "'";
