<?php
	...
	public function index(){
		...
		$this->document->addScript('view/javascript/onepagecheckout/jquery-ui.js');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/onepagecheckout', $token_prefix . '=' . $this->session->data[$token_prefix], $this->ssl));
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('common/dashboard', $token_prefix . '=' . $this->session->data[$token_prefix], $this->ssl )
		);


		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/onepagecheckout', $token_prefix . '=' . $this->session->data[$token_prefix], $this->ssl)
		);

		$data['store_action'] =  $this->url->link('extension/onepagecheckout',$token_prefix . '=' . $this->session->data[$token_prefix], $this->ssl);
		
		$data['action'] = $this->url->link('extension/onepagecheckout', $token_prefix . '=' . $this->session->data[$token_prefix] . '&store_id='. $data['store_id'], $this->ssl);

		$data['cancel'] = $this->url->link('extension/extension', $token_prefix . '=' . $this->session->data[$token_prefix], $this->ssl);
		
		$data['restoresetting'] = $this->url->link('extension/onepagecheckout/restoresetting', $token_prefix . '=' . $this->session->data[$token_prefix].'&store_id='. $data['store_id'], $this->ssl);
		...
		//$data['order_success'] =  $this->url->link('extension/order_success_page','&user_token='.$this->session->data[$token_prefix]);
		...
		//if(empty($data['onepagecheckout_manage']['shopping_cart']['shopping_cart_status'])) {
			//$data['onepagecheckout_manage']['shopping_cart']['shopping_cart_status'] = '1';
		//}
		
		if(empty($data['onepagecheckout_manage']['shopping_cart'])) {
			$data['onepagecheckout_manage']['shopping_cart'] = array(
				...
				'show_product_unit' => '1',
				'show_product_total_price' => '1',
			);
		}
		
		if(empty($data['onepagecheckout_manage']['personaldetails']['guest_status'])) {
			$data['onepagecheckout_manage']['personaldetails']['guest_status'] = array(
				'guest_status' => '1',
			);
		}
		
		if(empty($data['onepagecheckout_manage']['general'])) {
			$data['onepagecheckout_manage']['general'] = array(
				'account_open' => 'guest',
			);
		}
		
		if(empty($data['onepagecheckout_manage']['confirm_order'])) {
			$data['onepagecheckout_manage']['confirm_order'] = array(
				'comment_status' => '1',
			);
		}
		...
		//$data['customfeilds'] = $this->url->link('customer/custom_field','&user_token='.$this->session->data[$token_prefix]);
		
		$this->load->model('tool/image');	
		$this->load->model($model_extension_load);

		$delivery_methods = $this->$model_extension_path->getInstalled('shipping');
		
		$payment_methods = $this->$model_extension_path->getInstalled('payment');
		
		$data['delivery_methods'] = array();
		foreach($delivery_methods as $delivery_method_code){
			if ($this->config->get($shipping_prefix.$delivery_method_code . '_status')) {
		...
		foreach($payment_methods as $payment_method_code){
			if ($this->config->get($payment_prefix.$payment_method_code . '_status')) {
		...
		$extensions = $this->$model_extension_path->getInstalled('dashboard');
	...
	public function Restoresetting(){
		...
		if($this->validate()){
			...
			$file = DIR_APPLICATION.'view/javascript/onepagecheckout/onepagecheckout.sql';
			...
			if(file_exists($file)){
				...
				$this->session->data['success'] = 'Success: You have restored default checkout settings';
			}else{
