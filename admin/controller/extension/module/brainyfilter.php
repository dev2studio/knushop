...
class ControllerExtensionModuleBrainyFilter extends Controller {
    ...
    private $default = array(
        'basic' => array(
            'behaviour' => array(
                'containerSelector' => '.grid-holder.product-holder',
                'paginatorSelector' => '.pagination-holder',
                'attribute_groups' => 1,
                'product_count' => 1,
				'rounded_img' => 1,
                'hide_empty' => 0,
    ...
    public function install() {
		...
		if ((float)VERSION >= 3.0) {
			$model_event_load = 'setting/event';
			$model_event_path = 'model_setting_event';
			$token_prefix = 'user_token';
			$this->load->model('setting/modification');
			$modification_prefix = 'model_setting_modification';
		} else {
			$model_event_load = 'extension/event';
			$model_event_path = 'model_extension_event';
			$token_prefix = 'token';
			$this->load->model('extension/modification');
			$modification_prefix = 'model_extension_modification';
		}

		$xml = file_get_contents(DIR_APPLICATION . '../admin/view/javascript/brainyfilter/ocmod/brainyfilter.ocmod.xml');
		$data = array(
            'name' => 'Brainy Filter',
            'code' => 'brainyfilter',
			'id' => 'brainyfilter',
			'extension_install_id' => 'brainyfilter',
		...
		$mod = $this->$modification_prefix->getModificationByCode('brainyfilter');
        if (!$mod) {
        $this->$modification_prefix->addModification($data);
		}

		$this->load->model($model_event_load);
        ...
        $this->$model_event_path->addEvent('brainyfilter', 'admin/model/catalog/product/addProduct/after', 'extension/module/brainyfilter/eventAddProduct');
        $this->$model_event_path->addEvent('brainyfilter', 'admin/model/catalog/product/editProduct/after', 'extension/module/brainyfilter/eventEditProduct');
        $this->$model_event_path->addEvent('brainyfilter', 'admin/model/catalog/product/deleteProduct/after', 'extension/module/brainyfilter/eventDeleteProduct');
        $this->$model_event_path->addEvent('brainyfilter', 'admin/model/catalog/review/addReview/after', 'extension/module/brainyfilter/eventEditReview');
        $this->$model_event_path->addEvent('brainyfilter', 'admin/model/catalog/review/editReview/after', 'extension/module/brainyfilter/eventEditReview');
        $this->$model_event_path->addEvent('brainyfilter', 'admin/model/catalog/review/deleteReview/after', 'extension/module/brainyfilter/eventEditReview');
    }
    ...
    public function uninstall() {
        ...
        $this->$model_event_path->deleteEvent('brainyfilter');
    }
    ...
    public function index() {
		...
		$this->document->addScript('view/javascript/brainyfilter/tinycolor.js');
		$this->document->addScript('view/javascript/brainyfilter/jquery.fn.sortable.min.js');
		$this->document->addScript('view/javascript/brainyfilter/bootstrap.colorpickersliders.min.js');
		$this->document->addScript('view/javascript/brainyfilter/jquery.autocomplete.min.js');
		$this->document->addScript('view/javascript/brainyfilter/brainyfilter.js');
        ...
        if (isset($this->request->post['bf'])) {
            ...
            if ($this->_validate($post)) {
                ...
                if ($this->request->post['action'] == 'apply') {
                    $this->response->redirect($this->url->link('extension/module/brainyfilter', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=' . $moduleId, 'SSL'));
                } else {
                    $this->response->redirect($this->url->link($modules_url, $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL'));
                }
            }
        }
        ...
        $this->_data['breadcrumbs'] = array();
   		...
   		$this->_data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('common/home', $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL'),
      		'separator' => false
   		);

   		$this->_data['breadcrumbs'][] = array(
       		'text'      => $this->language->get('module'),
			'href'      => $this->url->link($modules_url, $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL'),
      		'separator' => ' :: '
   		);
		
   		$this->_data['breadcrumbs'][] = array(
       		'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('extension/module/brainyfilter', $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL'),
      		'separator' => ' :: '
   		);
        ...
        $this->_data['cancel'] = $this->url->link($modules_url, $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL');
        $this->_data['action'] = $this->url->link('extension/module/brainyfilter', $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL');
        $this->_data['instanceUrl'] = $this->url->link('extension/module/brainyfilter', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=', 'SSL');
        $this->_data['refreshAction'] = $this->url->link('extension/module/brainyfilter/refresh', $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL');
        $this->_data['attributeValuesAction'] = $this->url->link('extension/module/brainyfilter/attributeValues', $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL');
        $this->_data['modRefreshAction'] = $this->url->link('extension/module/brainyfilter/modRefreshTrigger', $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL');
        $this->_data['removeInstanceAction'] = $this->url->link('extension/module/brainyfilter/removeInstance', $token_prefix . '=' . $this->session->data[$token_prefix] . '&module_id=' . $moduleId, 'SSL');
        ...
        if ($moduleId !== 'basic' && $moduleId !== 'new') {
            $curModule = $this->$model_module_path->getModule($moduleId);
        ...
        $this->_data['modules'] = $this->$model_module_path->getModulesByCode('brainyfilter');
    ...
    public function removeInstance() {
        ...
        if ($moduleId && $moduleId !== 'basic' && $moduleId !== 'new') {
            ...
            $this->load->model($model_module_load);
			$this->$model_module_path->deleteModule($moduleId);
        }
        
        $this->response->redirect($this->url->link('extension/module/brainyfilter', $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL'));
    }
    ...
    private function _saveSettings($data) {
        ...
        else 
        {
			...
			if (isset($data['behaviour']['rounded_img']) && $data['behaviour']['rounded_img'] == 2) {
                unset($data['behaviour']['rounded_img']);
            }
			...
			$this->load->model($model_module_load);
            if ($moduleId === 'new') {
                $this->$model_module_path->addModule('brainyfilter', $module);
                $moduleId = $this->db->getLastId();
            } else {
                $this->$model_module_path->editModule($moduleId, $module);
            }
...
//        }
    ...
    private function _applySettings() {
        ...
        if (isset($this->request->post['bf']) && is_array($this->request->post['bf'])) {
			...
			$this->load->model($model_module_load);
            $allModules = $this->$model_module_path->getModulesByCode('brainyfilter');
    ...
    public function ocmodManager()
    {
        ...
        if (isset($this->request->post['bf'])) {
            ...
            $this->response->redirect($this->url->link('extension/module/brainyfilter/ocmodManager', $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL'));
        }
    ...
    public function modRefreshTrigger()
    {
        ...
        if ($this->model->enableMod($enable)) {
            $this->response->redirect($this->url->link('extension/modification/refresh', $token_prefix . '=' . $this->session->data[$token_prefix], 'SSL'));
        }
    }
