...
'thumb' => $this->model_tool_image->resize($product_image, 50, 50),
						'option'   => $option_data,
				...
				if($order_info['payment_code'] =='cheque'){
					$data['text_instruction'] = $language->get('text_pays_1');
					$data['comment'] = $language->get('text_new_instruction');
				}else{

				}


 				//$data['comment'] = 'ewrwere'.$order_info['payment_code'];
				$text .= $language->get('text_new_footer') . "\n\n";



				$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_simple_fields`  WHERE order_id = '" . (int)$order_id . "'");
				if ($order_query->row) {
					$item = $order_query->row;

					$data['field27'] = $item['field27'];
