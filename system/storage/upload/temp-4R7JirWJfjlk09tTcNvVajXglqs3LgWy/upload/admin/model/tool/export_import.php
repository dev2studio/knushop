...
//		$col -= 1; // we use 1-based, PHPExcel uses 0-based column index, PhpSpreadsheet now uses 1-based column index
	...
	protected function setColumnStyles( &$worksheet, &$styles, $min_row, $max_row ) {
		...
		foreach ($styles as $col=>$style) {
			$from = PhpOffice\PhpSpreadsheet\Cell\Cell::stringFromColumnIndex($col).$min_row;
			$to = PhpOffice\PhpSpreadsheet\Cell\Cell::stringFromColumnIndex($col).$max_row;
			$range = $from.':'.$to;
	...
	protected function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$numberFormat = $worksheet->getCell('P2',false)->getStyle()->getNumberFormat();
		ob_start();var_dump($numberFormat);$this->log->write("populateProductsWorksheet: numberFormat = '".ob_get_contents()."'");ob_end_clean();
	}
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents("http://www.mhccorp.com/index.php?route=information/message&type=tool_export_import_4_4&language_code=$language_code");
