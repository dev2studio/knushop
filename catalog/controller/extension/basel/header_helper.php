<?php
	...
	$data['basel_custom_js'] = $this->config->get('basel_custom_js');
	...
	// Mandatory CSS
	if ($this->cache->get('basel_mandatory_css')) {
		$data['basel_mandatory_css'] = $this->cache->get('basel_mandatory_css');
		} else {
		...
		$this->cache->set('basel_mandatory_css', $madatory_css);
		$data['basel_mandatory_css'] = $this->cache->get('basel_mandatory_css');
	}	
	...
	$data['basel_search_scheme'] = $this->config->get('basel_search_scheme');
		if ($this->cache->get('basel_styles_cache')) {
		$data['basel_styles_cache'] = $this->cache->get('basel_styles_cache');
		} else {
		...
		$styles .= 'div.ui-slider-range.ui-widget-header, .bf-slider-range.ui-slider .ui-slider-handle {background:' . $this->config->get('basel_primary_accent_color') . ' !important;}';
		...
		$this->cache->set('basel_styles_cache', $styles);
		$data['basel_styles_cache'] = $this->cache->get('basel_styles_cache');
		}
	}
	...
	if ($this->cache->get('basel_fonts_cache')) {
	$data['basel_fonts_cache'] = $this->cache->get('basel_fonts_cache');
	} else {
	...
	$this->cache->set('basel_fonts_cache', $font_styles);
	$data['basel_fonts_cache'] = $this->cache->get('basel_fonts_cache');
	} 
	} else {
