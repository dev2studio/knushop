...
class ControllerExtensionOnepagecheckoutPaymentMethod extends Controller {
	public function index() {
		...
		if (isset($this->session->data['payment_address'])){
			...
			$results = $this->$model_extension_path->getExtensions('payment');

			$recurring = $this->cart->hasRecurringProducts();
			
			foreach ($results as $result) {
				//if ($this->config->get($payment_prefix . $result['code'] . '_status') && !empty($onepagecheckout_manage['payment_method'][$result['code']]['status'])) {
				if ($this->config->get($payment_prefix . $result['code'] . '_status')) {
