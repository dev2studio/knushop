 ...
 $.get('https://knushop.com.ua/black-knu-sticke',function(sdsd){

 	var wee = sdsd.split('<span class="cart-name1">');
 	    wee = wee[1].split('</span>');
 	    wee = wee[0];
...
$('div').find(".container").each(function(i,val){
		...
		$.ajax({
			...
			$('[data-toggle=\'tooltip\']').tooltip('hide');
				...
				if (json['success_redirect']) {
					...
					html += '<a class="btn btn-default btn-block 11" href="https://knushop.com.ua/index.php?route=checkout/cart">'+ $('.cart-name1').html()+'</a>';
					html += '</div>';
					html += '</div>';
					html += '</div>';
					$('body').append(html);
