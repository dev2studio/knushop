<div class="table product-info quickview-info hidden-xs" style="height:<?php echo $img_h; ?>px; width:980px;">
    ...
    <?php if ($price) { ?>
      ...
      <div id="product">
          ...
          <?php } else { ?>
          <input step="1" min="<?php echo $minimum; ?>" type="number" name="quantity" value="<?php echo $minimum; ?>" class="input-quantity quickview-modal__btn" id="input-quantity" class="form-control" />
          <input type="hidden" name="product_id" value="<?php echo $product_id; ?>" />

          <button type="button" onclick="cart.add('<?php echo $product_id; ?>', $('#input-quantity').val(), 'qv')" class="btn btn-primary quickview-modal__btn"><?php if (($qty < 1) && ($stock_badge_status)) { ?><?php echo $basel_text_out_of_stock; ?><?php } else { ?><?php echo $button_cart; ?><?php } ?></button>
          <?php } ?>

          <a href="<?php echo $product_href; ?>" class="btn btn-outline details quickview-modal__btn"><?php echo $basel_text_view_details; ?></a>
			<?php if($config_on_off_qo_product_page == '1') { ?>



<button class="btn btn-primary quickview-modal__quick-order" type="button" onclick="fastorder_open(<?php echo $product_id?>);" title="<?php echo $config_text_open_form_send_order[$lang_id]['config_text_open_form_send_order']; ?>" style="background-color: transparent;border-color: #b80000;color: #b80000;">
	...
	<?php } ?>
	<span><?php echo $config_text_open_form_send_order1; ?></span>
